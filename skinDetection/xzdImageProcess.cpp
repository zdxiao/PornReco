...
int randTwoPoints(vector<Point2i> points, int &index_point1, int &index_point2, double minPointsDistance = 10)
{
...
void lineInlier(vector<Point2i> &points, Vec3d &_line, vector<Point2i> &inliers, 
				vector<Point2i> &outliers, double maxInlinerDis = 5)
{
	if(!inliers.empty())
	{
		inliers.clear();
						...
						double maxInlinerDis, Mat &garyImage, vector<Point2d> &linesMidPoint)
{
	...
	srand(time(NULL));
	vector<Point2i> points(edgePoints);
	...
	while(iter < max_iteration && detectedLinesNum < lineNum && points.size() > minLineLength)
	{
		...
		vector<Point2i> outliers, inliers;
...
void mergeParallelLines(vector<Vec3d> &lines, double angleThres, vector<Point2d> linesPoint)
{
	...
	vector<std::vector<int> > paralleLines;
	...
	delete [] tmp;
	delete [] fullSet;
	vector<Vec3d> newLines;
	for(vector<vector<int> >::iterator iter = paralleLines.begin(); iter != paralleLines.end(); ++iter)
	{
		Vec3d tmpLine(0, 0, 0);
